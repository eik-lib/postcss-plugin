...
tap.test(
    ...
    (t) => {
        t.rejects(
            ...
            new Error(
                'Import specifier can NOT be mapped to a bare import statement. Import specifier "foo" is being wrongly mapped to "./foo"',
            ),
        );
        t.end();
    },
                ...
                maps: [
                    {
                        imports: {
                            'normalize.css':
                                'https://cdn.eik.dev/normalize.css@8/normalize.css',
                        },
                    },
                ],
            }),
...
tap.test('plugin() - eik.json defined import maps', async (t) => {
    ...
    const { css } = await postcss(plugin()).process(simple, {
        from: undefined,
    });
    ...
    async (t) => {
        ...
        server.get('/one', (request, reply) => {
            reply.send({
                imports: {
                    'normalize.css':
                        'https://cdn.eik.dev/normalize.css@7/normalize.css',
                },
            });
        });
        const address = await server.listen({
        ...
        t.matchSnapshot(
            clean(css),
            'Should rewrite import statement to https://cdn.eik.dev/normalize.css@8/normalize.css',
        );
        await server.close();
        await fs.promises.unlink(path.join(process.cwd(), 'eik.json'));
        t.end();
    ...
    async (t) => {
        ...
        server.get('/one', (request, reply) => {
            reply.send({
                imports: {
                    'normalize.css':
                        'https://cdn.eik.dev/normalize.css@6/normalize.css',
                },
            });
        });
        server.get('/two', (request, reply) => {
